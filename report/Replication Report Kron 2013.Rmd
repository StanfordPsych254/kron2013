...
For the arousal scale, which ranged from calm (0) to aroused (8), participants were instructed that “one extreme represents feeling stimulated, excited, frenzied, jittery, wide-awake, or aroused; and at the other end of the scale, you feel completely relaxed, calm, sluggish, dull, sleepy, or bored.” In the separate unipolar-scales condition, par- ticipants followed the same instructions they had used for the bipolar valence scale except that they were instructed to rate their feelings using two separate scales, one rang- ing from no pleasant feelings (0) to strong pleasant feelings (8), and the other ranging from no unpleasant feelings (0) to strong unpleasant feelings (8)."
...
We found that both bipolar valence and arousal ratings in our sample [WERE/WERE NOT significantly] linearly correlated with normative IAPS ratings for each item (valence: *r* = `r signif(res1$coefficients["bipolarvalence","Estimate"],digits=3)`, *p* = `r signif(res1$coefficients["bipolarvalence","Pr(>|t|)"],digits=3)`; arousal: *r* = `r signif(res2$coefficients["arousal","Estimate"],digits=3)`, *p* = `r signif(res2$coefficients["arousal","Pr(>|t|)"],digits=3)`). This indicated [a low/moderate/high] association between the affective responses of our Mechanical Turk sample and Lang et al.'s normative ratings, suggesting that our sample was [not very/moderately/highly] representative of the broader population.
...
We found that there [WAS/WAS NOT] a significant linear correlation between arousal ratings and the summed unipolar pleasant+unpleasant ratings (*r* = `r signif(res3$coefficients["sum_pl_un","Estimate"],digits=3)`, *p* = `r signif(res3$coefficients["sum_pl_un","Pr(>|t|)"],digits=3)`
) (see Figure). 
...
We found that there [WAS/WAS NOT] a significant linear correlation between valence ratings and the difference of unipolar pleasant-unpleasant ratings (*r* = `r  signif(res4$coefficients["diff_pl_un","Estimate"],digits=3)`, *p* = `r signif(res4$coefficients["diff_pl_un","Pr(>|t|)"],digits=3)`
) (see Figure). 
We can compare this with Kron et al.'s reported finding of a near-perfect correlation between bipolar valence ratings and the difference between unipolar pleasant and unpleasant ratings (*r* = 0.96, *p* < 0.0001). 
