...
target_uni = c("pleasant","unpleasant")
d_spread_unipolar = d_filtered %>% 
  filter(rating==target_uni) %>%
  select(iaps_trial, subID, rating, rating_type, response) %>%
  spread(rating,response) %>%
  mutate(sum_pl_un = pleasant + unpleasant, diff_pl_un = pleasant - unpleasant) %>% #calculate sum and diff of unipolar ratings 
  mutate(arousal = NA, bipolarvalence = NA) %>%
  select(iaps_trial:rating_type, arousal, bipolarvalence, pleasant:diff_pl_un)

target_bi = c("bipolarvalence","arousal")
d_spread_bipolar = d_filtered %>%
  filter(rating==target_bi) %>%
  select(iaps_trial, subID, rating, rating_type, response) %>%
  spread(rating, response) %>%
  mutate(pleasant = NA, unpleasant = NA, sum_pl_un = NA, diff_pl_un = NA)

d_spread_all = rbind(d_spread_bipolar, d_spread_unipolar) 

d_regather_all = d_spread_all %>%
  group_by(iaps_trial,subID) %>%
  gather(rating, response, arousal:diff_pl_un)

d_regather_all$rating = factor(d_regather_all$rating)


#Models:
#mixed model with random intercept effects
#WON'T WORK: 
keyModel1 = lmer(arousal ~ sum_pl_un + (1|subID) + (1|iaps_trial), data=d_spread_all, REML = TRUE)
summary(keyModel1)
