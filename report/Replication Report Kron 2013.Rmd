...
####Import data
# set directory
#setwd("/Users/Maia/Desktop/Neuro/Stanford/Classes/PSYCH 254/replication_project/kron2013_gitrepo")
directory = "../data/"
...
```{r}
setwd("/Users/Maia/Desktop/Neuro/Stanford/Classes/PSYCH 254/replication_project/kron2013_gitrepo/analysis/")
...
Kron and colleagues modeled the relationship between self-reported bipolar and unipolar ratings with hierarchical mixed linear models: "Centered arousal scores and centered pleasant+unpleasant scores for each trial (72 points per participant) were entered in one vector as one predictor; we term this predictor ‘arousal/pleasant+unpleasant ratings scores'. At the same way, a second, continuous vector consists of centered bipolar valence and pleasant-unpleasant ratings scores enter as a second predictor and is termed ‘valence/pleasant-unpleasant scores'. A third, effect-coding vector (-1 and 1), was added to model. This vector coded for each of the scores in vectors 1 and 2, the model they are belong to: bipolar valence and arousal ratings were coded as -1 and pleasant+unpleasant, and pleasant-unpleasant were coded 1. We term this predictor “type of scale”. Subject intercept was entered as random effect."
"..[T]o ensure that our results were not unique to item analysis, we also examined them using hierarchical linear modeling (HLM) analysis (using the SAS PROC MIXED procedure)."

We therefore replicated their analysis with a mixed linear model. We entered three fixed effects: 
* 1) a variable "bipolarvalence/diff_pl_un" with two levels -- 0 for centered arousal/pleasant+unpleasant ratings and 1 for centered valence/pleasant-unpleasant ratings; 

* 2) a variable "arousal/sum_pl_un" with two levels -- 0 for centered valence/pleasant-unpleasant ratings and 1 for centered arousal/pleasant+unpleasant ratings; and 

* 3) a `rating_type` factor with two levels, bipolar scales (effect coded -1) and unipolar scales (effect coded as 1). 

We entered subject intercept and item intercept as random effects.
...
#factor variables
d_regather$predictor = factor(d_regather$predictor)
d_regather$rating_type = factor(d_regather$rating_type)
d_regather$arousal_sum_pl_un = factor(d_regather$arousal_sum_pl_un)
d_regather$bipolarvalence_diff_pl_un = factor(d_regather$bipolarvalence_diff_pl_un)

#NOT SURE I'VE GOT THIS RIGHT; THROWS ERROR:
#effect-code: bipolar = -1; unipolar = 1
# contrasts(d_regather$rating_type) = c(-1, 1)
# contrasts(d_regather$rating_type)
# 
# #effect-code: arousal/sum_pl_un = -1; bipolarvalence/diff_pl_un = 1
# contrasts(d_regather$predictor) = c(1, -1)
# contrasts(d_regather$predictor)

library(lme4)
library(lmerTest)
#Models: (NB aren't working with so few datapoints...)
#mixed model with random intercept effects
# keyModel1 = lmer(response ~ arousal_sum_pl_un + bipolarvalence_diff_pl_un + rating_type + (1|subID) + (1|iaps_trial), data=d_regather, REML = TRUE)
# print(keyModel1)
# summary(keyModel1)
# 
# #mixed model with random intercept and slope effects
# keyModel2 = lmer(response ~ arousal_sum_pl_un + bipolarvalence_diff_pl_un + rating_type + (1|subID) + (predictor|subID) + (rating_type|subID) + (1|iaps_trial) + (predictor|iaps_trial) + (rating_type|iaps_trial), data=d_regather, REML = TRUE)
# summary(keyModel1)
...
We recreated this figure. We also plotted the relationship between the different of pleasant and unpleasant scores on the X axis and bipolar valence scales on the Y axis (this figure was not provided in the original paper, though the *r* correlation of 0.96 was provided).

```{r}
##NB: These aren't working properly yet (not enough data)
...
We compared participants' ratings against the normative IAPS ratings for our stimuli:
