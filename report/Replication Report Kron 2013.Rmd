...
We excluded participants who used mobile devices, browser heights less than 700 pixels, or browser widths less than 1000 pixels (indicating that they were not viewing the experiment in full screen). Participants who made the same rating on all trials were excluded for having not been appropriately attentive. Participants who made more a response in less than 300ms were excluded for not having been appropriately attentive. Participants who failed to answer the attention check question correctly at the end were excluded.
...
We [confirmed] that both bipolar valence and arousal ratings in our sample were [significantly] linearly correlated with normative IAPS ratings for each item (valence: *r* = `r res1$coefficients["bipolarvalence","Estimate"]`, *p* = `r res1$coefficients["bipolarvalence","Pr(>|t|)"]`; arousal: *r* = `r res2$coefficients["arousal","Estimate"]`, *p* = `r res2$coefficients["arousal","Pr(>|t|)"]`). This indicated that the affective responses of our Mechanical Turk sample [DID or DID NOT] generalize to the broader population.
...
We found that there [WAS/WAS NOT] a significant linear correlation between arousal ratings and the summed unipolar pleasant+unpleasant ratings (`r  res3$coefficients["sum_pl_un","Estimate"]`, *p* = `r res3$coefficients["sum_pl_un","Pr(>|t|)"]`
) (see Figure). 
This finding [DID/DID NOT] replicate Kron et al.'s reported finding (*r* = 0.75, *p* < 0.0001, *$R^2$* = 0.57). 
...
We found that there [WAS/WAS NOT] a significant linear correlation between valence ratings and the difference of unipolar pleasant-unpleasant ratings (`r  res4$coefficients["diff_pl_un","Estimate"]`, *p* = `r res4$coefficients["diff_pl_un","Pr(>|t|)"]`
) (see Figure). 
This finding [DID/DID NOT] replicate Kron et al.'s reported finding (*r* = -0.13, *p* > 0.05). 
