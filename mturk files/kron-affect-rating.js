// 3/5/17: Outstanding issues to fix:
// 1. images have to scroll down to see whole thing; not fixed by setting height to 100% or auto
// 2. possible to click through ratings without having made a response
// 3. sometimes "please make a response" appears without warning
// 4. iaps_trial is not being response_logged
// 5. create pseudorandom order of trials within subblocks, then randomize order of subblocks
// 6. sometimes one of the radio buttons is already clicked (especially when i haven't answered previous rating) = anchoring!
// 7. try forcing it into fullscreen...
// 8. run through whole experiment to see if demographics at end are working and data collected
// 9. optional: enter normative IAPS ratings so data is easier to deal with
// 10. checkmobile is not functioning properly, not being logged
// 11. subblock is counting up too high during the practice trials
...
// append practice photos onto IAPS length - append, say if 3 then done
...
window.mobilecheck = function() {
  ...
  (function(a){
   ...
   return checkmobile; // This isn't working!
};
...
var showRating = function() {
  ...
  if (scale_type[subblock] == "bipolar" && scale_order == "AB" && part_of_trial == 1) {
    ...
    var scale_content = "bipolarvalence";
...
function getiapsFile(image_num) {
  return 'http://web.stanford.edu/~mtb/IAPS_stimuli/' + image_num + '.jpg';
}
...
var experiment = {
    ...
    log_response: function() {
      ...
      var radio = document.getElementsByName("judgment");   // This isn't working properly
    ...
    next: function() {      
      ...
      if (window.self == window.top | turk.workerId.length > 0) {
          ...
          if (is_practice == true) {
            ...
            var iaps_trial = practice_iaps.shift();
            ...
            // Get the current trial - <code>shift()</code> removes the first element
            // select from our scales array and stop exp after we've exhausted all the domains
            var iaps_trial = iaps_order.shift();
